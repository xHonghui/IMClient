...
public class EaseNotifier {
	...
	private static EaseNotifier mInstance;
	...
	public static EaseNotifier getInstance() {
		if (mInstance == null) {
			throw new IllegalStateException("请先调用EaseNotifier的init()方法");
		}
		return mInstance;
	}
	...
	public static void init(Context context) {
		if (mInstance == null) {
			mInstance = new EaseNotifier(context);
		}
	}
	...
	public synchronized void onNewMesg(List<EMMessage> messages) {
		if (EMClient.getInstance().chatManager().isSlientMessage(
	...
	protected void sendNotification(EMMessage message, boolean isForeground,
		...
		try {
			...
			String summaryBody = msgs[6].replaceFirst("%1",
					Integer.toString(fromUsersNum)).replaceFirst("%2",
	...
	public void setNotificationInfoProvider(
			EaseNotificationInfoProvider provider) {
